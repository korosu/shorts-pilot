...
def _resolve_path(jobs_dir: Path, suffix: str, lang: str) -> Path:
    ...
    if old_p.exists():
        print(f"[migration] Found {old_p.name} but {new_p.name} is the expected "
...
def _warn_if_missing_defaults(path: Path, data: dict) -> None:
    """Warn if jobs file is missing defaults section or video_language field."""
    if "defaults" not in data:
        print(
            f"[warning] {path.name} has no 'defaults:' section.\n"
            f"  This file was likely bootstrapped from empty — mpt-batch\n"
            f"  will generate videos without language, aspect, or subtitle\n"
            f"  settings. Add a defaults block with at least video_language:\n"
            f"  \n"
            f"  defaults:\n"
            f"    video_language: \"en\"\n"
            f"    video_aspect: \"9:16\"\n"
    ...
    if not isinstance(defaults, dict):
        print(f"[warning] {path.name} has 'defaults:' but it's not a mapping")
        return

    if "video_language" not in defaults:
        print(
            f"[warning] {path.name} defaults: has no 'video_language' field.\n"
            f"  mpt-batch may default to the wrong output language."
        )
...
def load(jobs_dir: Path, suffix: str, lang: str) -> dict[str, Any]:
    ...
    if not p.exists():
        raise FileNotFoundError(
            ...
            f"Create {p.name} in your jobs directory first."
        )
    ...
    _warn_if_missing_defaults(p, data)
    return data
...
def _bootstrap_suffix(content: str, path: Path) -> str | None:
    ...
    try:
        ...
        raise ValueError(
            f"Refusing to append: {path.name} is not valid YAML ({e}). "
